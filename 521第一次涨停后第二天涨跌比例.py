...
# 统计第一次涨停后，第二日上涨与下跌的数量，值统计数量，不考虑收益和交易成本。
        ...
        if ooo!=[] and (lenn:=len(ooo))>=3:
            ...
            while i<lenn-1:
                if dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":  # 普通股票
                    if float_000(ooo[i-1][12])<0 and float_000(ooo[i][12])>=9.8:  # 普通股票第一次上涨按>=9.8
                    ...
                    if float_000(ooo[i-1][12])<0 and float_000(ooo[i][12])>=19.8:  # 科创板股票第一次上涨按>=19.8
                        if float_000(ooo[i+1][12])>0:  # 第二日上涨
                            rrr["A"]+=1
                            i+=2
                        elif float_000(ooo[i+1][12])<0:  # 第二日下跌
...
print(f"第一次涨停后，第二日上涨：{rrr['A']}，下跌：{rrr['V']}，上涨与下跌之比为：{round(rrr['A']/rrr['V'],2)}")
print("--end--")
