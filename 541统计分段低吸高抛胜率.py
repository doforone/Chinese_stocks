...
for dd in ddd:
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0] not in ddd3 and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":  # 可调整控制
        ...
        if (lenn:=len(ooo))>8:  # 一般上市新股前5天没有涨跌停，所以5+3
            ...
            T_n=0  #本交易次数
            for i in range(MN+1,lenn):
                if cang==0:
                    if float_000(ooo[i-1][12])<-max([abs(float_000(x[12])) for x in ooo[i-MN-1:i-1]])*AB:  # 因为每天买或卖只一次，所以用涨跌幅，参数可调
                        cang=T_moneyy/float_000(ooo[i][2])  # 开盘价买入，不考虑集合竞价因素及开盘价是否跳多还是跳空
                else:
                    if float_000(ooo[i-1][12])>max([abs(float_000(x[12])) for x in ooo[i-MN-1:i-1]])*AB or i==lenn-1:  # 上涨卖出，参数可调
