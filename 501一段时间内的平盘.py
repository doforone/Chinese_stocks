...
##                 x[12], x[7], x[8], x[10]] for x in ooo]
            ...
            try:
                vvv=[x[9]/x[8] for x in uuu[-MN:]]
                #rrr[dd[0]]=abs(max(vvv)/min(vvv)-1)
                if dd[0] in ddd3:
                    rrr.append([dd[0], dd[1], ddd2[dd[0]], abs(max(vvv)/min(vvv)-1), uuu[-1][0], uuu[-1][5], 1])
                else:
                    rrr.append([dd[0], dd[1], ddd2[dd[0]], abs(max(vvv)/min(vvv)-1), uuu[-1][0], uuu[-1][5], 0])
            except Exception as e:
                print(e)
                pass


print("说明：1、数据使用为不复权数据；2、当日均价=当日成交额/当日成交量；3、每年分红股票（●）为至少上市4年，最多1年没有分红（除权）的股票；4、新上市股票不定期更新。\r\n")
print(f"--------{MN}日振幅最小排名（前12）--------\r\n")
rrr=sorted(rrr, key=lambda x: x[3])
for rr in rrr[:12]:
    if rr[0][:6]=="sh.688":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） {MN}日振幅：{round(rr[3]*100,2)}%，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  科创板")
    elif rr[0][:4]=="sz.3":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） {MN}日振幅：{round(rr[3]*100,2)}%，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  创业板")
    else:
        print(f"{rr[0]} {rr[1]}（{rr[2]}） {MN}日振幅：{round(rr[3]*100,2)}%，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}")
print("......")

print(f"========{MN}日振幅最大排名（前12）========\r\n")
for rr in rrr[::-1][:12]:
    if rr[0][:6]=="sh.688":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） {MN}日振幅：{round(rr[3]*100,2)}%，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  科创板")
    elif rr[0][:4]=="sz.3":
