...
for oo in ooo:
    ...
    if oo[0][:3]!="of.":
        ...
        min0=min([min1,min2])
        yyy1=[x-min0 for x in yyy1]
        yyy2=[x-min0 for x in yyy2]
        ...
        plt.plot(xxx[:4600], yyy1[:4600], color="#000000", linewidth=1, label="不复权")
        plt.fill_between(xxx[:4600], yyy1[:4600], color='#000000',alpha=0.5)
        plt.plot(xxx[:4600], yyy2[:4600], color="#ffff00", linewidth=1, label="前复权")
        plt.fill_between(xxx[:4600], yyy2[:4600], color='#ffff00',alpha=0.5)
        ...
        plt.savefig(f"img_d_不复权叠加前复权2/{oo[0]}_{oo[1].replace('*','x')}（{classs}）.png", dpi=72, bbox_inches = 'tight')
