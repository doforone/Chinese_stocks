...
for dd in ddd:
    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
        ...
        if os.path.exists(f'data/k_line_{a}/{dd[0]}_{a}.txt'):
            ...
            if len(ooo)>abs(s-7):
                if float_000(ooo[s-1][10])!=0:
                    市值=float_000(ooo[s-1][5])*(float_000(ooo[s-1][7])*100/float_000(ooo[s-1][10]))
                    AV=(float_000(ooo[s-1][5])-float_000(ooo[s-7][2]))*100/float_000(ooo[s-7][2])
                    rrr.append([dd[0], dd[1], ddd2[dd[0]], ooo[-1][0], 市值, AV])
...
for rr in rrr[:300]:
    AV+=rr[5]
    print(f"{rr[0]} {rr[1]}（{rr[2]}） 数据截止：{rr[3]}  市值：{int(rr[4]/10000):,}万  7日涨跌：{rr[5]:.2f}%")
