...
for dd in ddd:
    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0] not in ddd3 and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
        ...
        if ooo!=[] and (lenn:=len(ooo))>=6+1000:
            i=4+1000
            while i<lenn-1-0:
                if float_000(ooo[i-4][12])>0 and float_000(ooo[i][12])<float_000(ooo[i-1][12])<float_000(ooo[i-2][12])<float_000(ooo[i-3][12])<0\
                   and -8<float_000(ooo[i][12])<-7:  # 第一次连续三日加速下跌
##                if float_000(ooo[i-4][12])>0 and float_000(ooo[i-3][12])<float_000(ooo[i-2][12])<float_000(ooo[i-1][12])<float_000(ooo[i][12])<0\
##                   and -2<float_000(ooo[i][12])<-0:  # 第一次连续三日减速下跌
