...
print("========连续上涨超过3天，按天数排名（前50）========\r\n")

for rr in rrr[::-1][:50]:
    if rr[0][:6]=="sh.688":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  科创板")
    elif rr[0][:4]=="sz.3":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  创业板")
    else:
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}")
