...
for dd in ddd:
    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3" and dd[0] not in ddd4 and dd[1].find("ST")==-1:
        ...
        if os.path.exists(f'data/k_line_d_前复权_计算获得/{dd[0]}_d.txt'):
            ...
            mn=8  # 从第几天开始
            for i,uu in enumerate(uuu[mn:]):
                if buyy==0:
                    #if uuu[i+mn-3][7]<0 and uuu[i+mn-2][7]>0 and uuu[i+mn-1][7]>0:
                    if uuu[i+mn-3][7]<0 and 0<uuu[i+mn-2][7]<uuu[i+mn-1][7]<uuu[i+mn][7]:  # 加速上涨
                        if 9<=uu[7]<=10.2:
                            if uuu[i+mn+1][3]<=uu[5]:
                                buyy=uu[5]  # 以今日收盘价进行集合竞价买入
                                AV=9
                                #print(buyy)
                                print("+++",uu[7],uu[5],uu[0],dd[0])
                        elif 0<=uu[7]<9:
                            buyy=uu[5]  # 以今日收盘价进行集合竞价买入
                            AV=int(uu[7])
                            #print(buyy)
...
print(f"当日上涨超过9%，第一次下跌后累积涨跌：{rrr9['累积']:.2f}，次数：{rrr9['次数']}，平均：{rrr9['累积']*100/rrr9['次数']:.2f}%")
print(f"当日涨跌在9%与8%(含)之间，第一次下跌后累积涨跌：{rrr8['累积']:.2f}，次数：{rrr8['次数']}，平均：{rrr8['累积']*100/rrr8['次数']:.2f}%")
print(f"当日涨跌在8%与7%(含)之间，第一次下跌后累积涨跌：{rrr7['累积']:.2f}，次数：{rrr7['次数']}，平均：{rrr7['累积']*100/rrr7['次数']:.2f}%")
print(f"当日涨跌在7%与6%(含)之间，第一次下跌后累积涨跌：{rrr6['累积']:.2f}，次数：{rrr6['次数']}，平均：{rrr6['累积']*100/rrr6['次数']:.2f}%")
print(f"当日涨跌在6%与5%(含)之间，第一次下跌后累积涨跌：{rrr5['累积']:.2f}，次数：{rrr5['次数']}，平均：{rrr5['累积']*100/rrr5['次数']:.2f}%")
print(f"当日涨跌在5%与4%(含)之间，第一次下跌后累积涨跌：{rrr4['累积']:.2f}，次数：{rrr4['次数']}，平均：{rrr4['累积']*100/rrr4['次数']:.2f}%")
print(f"当日涨跌在4%与3%(含)之间，第一次下跌后累积涨跌：{rrr3['累积']:.2f}，次数：{rrr3['次数']}，平均：{rrr3['累积']*100/rrr3['次数']:.2f}%")
print(f"当日涨跌在3%与2%(含)之间，第一次下跌后累积涨跌：{rrr2['累积']:.2f}，次数：{rrr2['次数']}，平均：{rrr2['累积']*100/rrr2['次数']:.2f}%")
print(f"当日涨跌在2%与1%(含)之间，第一次下跌后累积涨跌：{rrr1['累积']:.2f}，次数：{rrr1['次数']}，平均：{rrr1['累积']*100/rrr1['次数']:.2f}%")
print(f"当日涨跌在1%与0%(含)之间，第一次下跌后累积涨跌：{rrr0['累积']:.2f}，次数：{rrr0['次数']}，平均：{rrr0['累积']*100/rrr0['次数']:.2f}%")
