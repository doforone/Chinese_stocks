...
for dd in reversed(ddd):
    ...
    if dd[0][:3]!="of.":
    #if dd[0][:3]=="sh.":
        if os.path.exists(f'data/K_line_d_前复权/{dd[0]}_{a}.txt'):
            with open(f'data/K_line_d_前复权/{dd[0]}_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
...
##            t_str="1900-01-01"
        ...
        # 前复权不同于不复权和后复权数据，每次需要从头开始下载，以上代码注销
        ooo=[]
        t_str="1900-01-01"
...
##            frequency=a, adjustflag="3")
        ...
        with open(f'data/K_line_d_前复权/{dd[0]}_{a}.txt', 'w', encoding='utf-8', newline='\r\n') as f:
