...
for dd in ddd:
    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0] not in ddd3 and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
        ...
        if ooo!=[]:
            ...
            if n>=3:  #加速下跌超过3天
                if dd[0] in ddd3:
                    rrr.append([dd[0], dd[1], ddd2[dd[0]], n, uuu[-1][0], uuu[-1][5], uuu[-1][7], 1])
                else:
                    rrr.append([dd[0], dd[1], ddd2[dd[0]], n, uuu[-1][0], uuu[-1][5], uuu[-1][7], 0])
...
rrr=sorted(rrr, key=lambda x: x[6])

print("========加速下跌（每天下跌均高于前一天）天数排名（前50）========\r\n")
for rr in rrr[::-1][:]:
    if rr[0][:6]=="sh.688":
        #print(f"{rr[0]} {rr[1]}（{rr[2]}） 加速下跌：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元，涨跌：【{rr[6]}】  {'●' if rr[7]==1 else ''}  科创板")
        pass
    elif rr[0][:4]=="sz.3":
        #print(f"{rr[0]} {rr[1]}（{rr[2]}） 加速下跌：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元，涨跌：【{rr[6]}】  {'●' if rr[7]==1 else ''}  创业板")
        pass
    else:
        #if rr[7]==1:
            print(f"{rr[0]} {rr[1]}（{rr[2]}） 加速下跌：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元，涨跌：【{rr[6]}】  {'●' if rr[7]==1 else ''}")
