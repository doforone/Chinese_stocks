...
for dd in reversed(ddd):
    #print(dd[0])
    #if dd[0][:3]!="of.":
    if dd[0][:3]!="of." and dd[5]=="1":
    #if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
    #if dd[0][:3]=="sh.":
        print(dd[0])
        #time.sleep(0.1)
        if os.path.exists(f'data/k_line_d/{dd[0]}_{a}.txt'):
            with open(f'data/k_line_d/{dd[0]}_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ...
            if ooo==[]:
                ...
                #if t_str=="2023-09-02":  #---------------这个日期为延后一天的
                    #continue  #---------------
        else:
            ooo=[]
            t_str="1900-01-01"
        ...
        rs = bs.query_history_k_data_plus(dd[0],
            ...
            frequency=a, adjustflag="3")  # adjustflag：复权类型，默认不复权：3；1：后复权；2：前复权。
        ...
        with open(f'data/k_line_d/{dd[0]}_{a}.txt', 'w', encoding='utf-8', newline='\r\n') as f:
