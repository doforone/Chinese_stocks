...
for dd in ddd:
    ...
    #if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":  # 一般使用
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[3]=="":  # AB量化web站点使用
    #if dd[0]=="sh.000001" or dd[0]=="sz.399001" or dd[0]=="sz.399006":
    #if 1==1:
        ...
        with open(f'data/k_line_{a}/{dd[0]}_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ...
            A98=round(sum(1 if float(oo[12])>=9.8 else 0 for oo in ooo if oo[12]!="")*100/len(ooo),2)
        ...
        for i in range(len(ooo)-1,-1,-1):  #==注意倒序
            if n%b==0:
                ...
                if h1==0:
                    ...
                    turn+=float(ooo[i][10])
                    high_low=(high-low)*2-abs(closee-openn)
                    #high_low=((high-low)*2-abs(closee-openn))/openn  #相对于开盘的百分百
...
##                n+=1
        ...
        if (lenn:=len(price_v))>2:
            ...
            for pp in price_v[::-1]:
                if pp==None:
                    ...
                    draw.line((i, 308, i, 308-int(((pp-minn)/maxx_minn)*300)), '#000000')
        ...
        if (lenn:=len(volume_v))>2:
            ...
            for vv in volume_v[::-1]:
                ...
                if vv==None:
                    ...
                    draw.line((i, 312, i, 312+int(((vv-minn)/maxx_minn)*300)), '#0033ff')
        ...
        if len(price_v)>2:
            ...
            text_size = setFont.getsize(f"{dd[0]} {dd[1]}（{classs}）_{b}{a} ({ttt1}, {ttt2}) ({h_l_2}) {ooo[-1][5]}元 [{A98}%]  \
{ooo[-3][12][:-2]}、{ooo[-2][12][:-2]}、{ooo[-1][12][:-2]}")
            text_size2 = setFont.getsize(f"({h_l_1})")
            #print(text_size)
            draw.text(((width-text_size[0])/2, 0), f"{dd[0]} {dd[1]}（{classs}）_{b}{a} ({ttt1}, {ttt2}) ({h_l_2}) {ooo[-1][5]}元 [{A98}%]  \
{ooo[-3][12][:-2]}、{ooo[-2][12][:-2]}、{ooo[-1][12][:-2]}", font=setFont, fill=fillColor)
            ...
            VVV=""
            for i in range(-3,0):
                if float_000(ooo[i][12])>0:
                    VVV+="A"
                elif float_000(ooo[i][12])==0:
                    VVV+="O"
                else:
                    VVV+="V"
...
##            print(price2_v[0])
##            if price2_v[0]!=None:
##                if price2_v[0]<max([x for x in price2_v if x!=None])/2 and VVV!="AAA":
##                    if volume_v[0]!=None and volume_v[0]>max([x for x in volume_v if x!=None])/3:
##                        img.save(f"img_{a}{b}_成交额/{title_a}{title_v}{dd[0]}_{dd[1].replace('*','x')}({classs})_{b}{a}({ooo[-1][5][:-2]}元){A98}%_{VVV}.png", "png")

            #img.save(f"img_{a}{b}_成交额/{title_a}{title_v}{dd[0]}_{dd[1].replace('*','x')}({classs})_{b}{a}({ooo[-1][5][:-2]}元){A98}%_{VVV}.png", "png")
            
            file_name=fun.md5(f"{dd[0]}_qwo6k7qk6522ran8rstrgmoxvvmqde")
            img.save(f"img_md5_0/{file_name}.png", "png")
