...
##                n+=1
        ...
        if len(price_v)>2:
            ...
            text_size2 = setFont.getsize(f"({h_l_1})  会员可见【{end_ttt}】及之后数据")
            #print(text_size)
            draw.text(((width-text_size[0])/2, 0), f"{dd[0]} {dd[1]}（{classs}）_{b}{a} ({ttt1}, {ttt2}) ({h_l_2}) {ooo[-1][5]}", font=setFont, fill=fillColor)
            draw.text(((width-text_size2[0])/2, height-20), f"({h_l_1})  会员可见【{end_ttt}】及之后数据", font=setFont, fill=fillColor)
            
            #Image1.show()
            #img.save(f"img_{a}2/{title_a}{title_v}{dd[0]}_{dd[1].replace('*','x')}（{classs}）_{b}{a}.png", "png")
            #file_name=fun.md5(f"{dd[0]}_{dd[1]}（{classs}）")
