...
for s in range(-500,-1):
        ...
        if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3" and dd[0] not in ddd4:
            ...
            if os.path.exists(f'data/k_line_d_东方财富_前复权/{dd[0]}_d.txt'):
                ...
                if (lenn:=len(ooo))>=abs(s):
                    ...
                    if n>=3 and f0(ooo[-n+s][8])>0 and ooo[-n+s][0]==ooo_A[-n+s][0] and ooo[-1][0]==ooo_A[-1][0]:
                       #and any([0 if f0(x[8])<9.8 else 1 for x in ooo[-10+s:s]]):  #====================
                    #if n>=3 and (f0(ooo[-n+s][2])/f0(ooo[-n+s][1])-1)>0:  #--------------------
                        EE=sum([abs(f0(x[8])) for x in ooo])/lenn
                        if dd[0] in ddd3:
                            if s<-1:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][2]), f0(ooo[s][8]), 1, f0(ooo[s+1][8]), amountt, EE, capp, turnn, AAVV])
                            else:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][2]), f0(ooo[s][8]), 1, 0, amountt, EE, capp, turnn, AAVV])
                        else:
                            if s<-1:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][2]), f0(ooo[s][8]), 0, f0(ooo[s+1][8]), amountt, EE, capp, turnn, AAVV])
                            else:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][2]), f0(ooo[s][8]), 0, 0, amountt, EE, capp, turnn, AAVV])
    ...
    if os.path.exists(f'data/k_line_d_东方财富_前复权/sh.000001_d.txt'):  #使用不复权数据，上证
        ...
        if s<-1:
            print(f"\r\n上证：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%    {ooo[s+1][0]}，\
【{ooo[s+1][2]}】({f0(ooo[s+1][6])/100000000:.0f}亿)，{f0(ooo[s+1][8]):.2f}%")
        else:
            print(f"上证：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%")
    #====================
    if os.path.exists(f'data/k_line_d_东方财富_前复权/sz.399001_d.txt'):  #使用不复权数据，深证
        with open(f'data/k_line_d_东方财富_前复权/sz.399001_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ooo=json.loads(f.read())
        if s<-1:
            print(f"深证：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%    {ooo[s+1][0]}，\
【{ooo[s+1][2]}】({f0(ooo[s+1][6])/100000000:.0f}亿)，{f0(ooo[s+1][8]):.2f}%")
        else:
            print(f"深证：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%")
    #====================
    if os.path.exists(f'data/k_line_d_东方财富_前复权/sz.399006_d.txt'):  #使用不复权数据，创业板
        with open(f'data/k_line_d_东方财富_前复权/sz.399006_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ooo=json.loads(f.read())
        if s<-1:
            print(f"创业板：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%    {ooo[s+1][0]}，\
【{ooo[s+1][2]}】({f0(ooo[s+1][6])/100000000:.0f}亿)，{f0(ooo[s+1][8]):.2f}%")
        else:
            print(f"创业板：{ooo[s][0]}，【{ooo[s][2]}】({f0(ooo[s][6])/100000000:.0f}亿)，{f0(ooo[s][8]):.2f}%")
    #====================
    ...
    #if len(rrr)>=35:
    rrr=sorted(rrr, key=lambda x: x[9], reverse=True)  # 按7日额排名,倒序 =========正常
    #else:
    ...
    #for rr in rrr[:len(rrr)//2]:
        ...
        #if t_n<4 and 0<=rrr_turn.index([rr[0], rr[12]])<600 and rr[1].find("ST")==-1 and 5<=rr[5]<=50 and -9<rr[6]<-0:  #7日换手率
        ...
        #if t_n<3 and rr[1].find("ST")==-1 and 6<=rr[5]<=40 and -9<rr[6]<-0 and rrr_cap.index([rr[0], rr[11]])>=100:
        ...
        if t_n<4 and rr[1].find("ST")==-1 and 5<=rr[5]<=50 and -9<rr[6]<-0:
