...
for oo in ooo:
    ...
    if oo[0][:3]!="of.":
        ...
        for dd in ddd:
            xxx.append(dd[0])
            xxx_labels.append(dd[0][:4])
            yyy1.append(float(dd[5]))

        with open(f'data\\K_line_d_后复权\{oo[0]}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ddd=json.loads(f.read())
        ...
        plt.plot(xxx, yyy2, color="#cccc00", linewidth=1, label="后复权")
        plt.fill_between(xxx, yyy2, color='#cccc00',alpha=0.5)
        ...
        plt.title(f"{oo[0]} {oo[1]}（{classs}）_d 后复权叠加不复权 @ 白点数据", fontsize=22, fontproperties=font, color="#000000")
        ...
        plt.grid(True)
        #plt.xticks(xxx, xxx_labels, rotation=90)
        #plt.xticks(xxx, labels=xxx_labels)
        #plt.xticks(range(0,len(xxx)), xxx_labels)
        plt.xticks(rotation=90)
        plt.tick_params(labelsize=18)

        #plt.savefig("000.png",dpi=10,bbox_inches = 'tight')
        plt.savefig(f"img_d_后复权叠加不复权/{oo[0]}_{oo[1].replace('*','x')}（{classs}）.png", dpi=72, bbox_inches = 'tight')
