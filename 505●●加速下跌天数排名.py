...
#s = -2  # 一般为-1，不得大于-1
for s in [-2,-1]:
#for s in [-260, -25, -197, -64, -275, -134, -184, -184, -72, -222, -235, -174, -287, -59, -50, -80, -116, -164, -169, -228]:
#for s in range(-307,-7):
#for s in range(-607,-307):
#for s in range(-907,-607):
#for s in range(-1207,-907):
    print(f"****************************************({s})****************************************")
    rrr=[]
        ...
        if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3" and dd[0] not in ddd4:
            ...
            if os.path.exists(f'data/k_line_{a}/{dd[0]}_{a}.txt'):
                ...
                if (lenn:=len(ooo))>=abs(s):
                    if s==-1:
                        amountt=sum([f0(x[8]) for x in ooo[s-6:]])  # 包括s日的交易额
                    else:
                        amountt=sum([f0(x[8]) for x in ooo[s-6:s+1]])  # 包括s日的交易额
                    ...
                    if f0(ooo[s][10])!=0:
                        capp=f0(ooo[s][5])*(f0(ooo[s][7])*100/f0(ooo[s][10]))  # s日的市值
                    else:
                        capp=0
                    rrr_cap.append([dd[0], capp])
                    
                    if (f12:=f0(ooo[s][12]))>0:
                        rrr2["A"]+=1
                    elif f12<0:
                        rrr2["V"]+=1
                    else:
                    ...
                    x=-1000
                    for oo in ooo[s::-1]:
                        if f0(oo[12])<0 and f0(oo[12])>x:  # 加速下跌
                        #if f0(oo[12])<0:  # 连续下跌
                            n+=1
                            x=f0(oo[12])
                        else:
                    ...
                    #if n>=3:  #加速下跌超过3天
                    if n>=3 and f0(ooo[-n+s][12])>0:
                        EE=sum([abs(f0(x[12])) for x in ooo])/lenn
                        if dd[0] in ddd3:
                            if s<-1:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][5]), f0(ooo[s][12]), 1, f0(ooo[s+1][12]), amountt, EE, capp, turnn])
                            else:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][5]), f0(ooo[s][12]), 1, 0, amountt, EE, capp, turnn])
                        else:
                            if s<-1:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][5]), f0(ooo[s][12]), 0, f0(ooo[s+1][12]), amountt, EE, capp, turnn])
                            else:
                                rrr.append([dd[0], dd[1], ddd2[dd[0]], n, ooo[s][0], f0(ooo[s][5]), f0(ooo[s][12]), 0, 0, amountt, EE, capp, turnn])
    ...
    if os.path.exists(f'data/k_line_{a}/sh.000001_{a}.txt'):  #使用不复权数据，上证
        with open(f'data/k_line_{a}/sh.000001_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ooo=json.loads(f.read())
        if s<-1:
            print(f"\r\n上证：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%    {ooo[s+1][0]}，【{ooo[s+1][5]}】，{f0(ooo[s+1][12]):.2f}%")
        else:
            print(f"上证：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%")
    #====================
    if os.path.exists(f'data/k_line_{a}/sz.399001_{a}.txt'):  #使用不复权数据，深证
        with open(f'data/k_line_{a}/sz.399001_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ooo=json.loads(f.read())
        if s<-1:
            print(f"深证：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%    {ooo[s+1][0]}，【{ooo[s+1][5]}】，{f0(ooo[s+1][12]):.2f}%")
        else:
            print(f"深证：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%")
    #====================
    if os.path.exists(f'data/k_line_{a}/sz.399006_{a}.txt'):  #使用不复权数据，创业板
        with open(f'data/k_line_{a}/sz.399006_{a}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ooo=json.loads(f.read())
        if s<-1:
            print(f"创业板：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%    {ooo[s+1][0]}，【{ooo[s+1][5]}】，{f0(ooo[s+1][12]):.2f}%")
        else:
            print(f"创业板：{ooo[s][0]}，【{ooo[s][5]}】，{f0(ooo[s][12]):.2f}%")
    #====================
    ...
    #for rr in rrr[:len(rrr)//2]:
        ...
        #if 0<=rrr_amount.index([rr[0], rr[9]])<500 and rr[1].find("ST")==-1 and 0<=rr[5]<8 and -9<rr[6]<-0:
            nn+=1
            ss+=rr[6]
            ss2+=rr[8]
            if rr[0][:6]=="sh.688":
