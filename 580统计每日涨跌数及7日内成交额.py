    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
        if os.path.exists(f'data/k_line_{a}/{dd[0]}_{a}.txt'):
            ...
            for ii,vv in enumerate(ooo):
                if ii>=11:  # 前5个交易日+7日内交易额
                    if vv[0] in rrr.keys():
                        pass
                    else:
                        rrr[vv[0]]={"A": 0, "V": 0, "D7": [], "VVV":[]}
                    ...
                    rrr[vv[0]]["D7"].append([dd[0], sum([f0(x[8]) for x in ooo[ii-6: ii+1]])])  # 包括s日的交易额
                    ...
                    if n>=3:
                        rrr[vv[0]]["VVV"].append([dd[0], f0(vv[12])])


for rr in rrr.keys():
    rrr[rr]["D7"].sort(key=lambda x: x[1], reverse=True)
    rrr[rr]["D7"]=[x[0] for x in rrr[rr]["D7"]]

    rrr[rr]["VVV"].sort(key=lambda x: x[1], reverse=False)  # 因为是负数，所以从小到大
    rrr[rr]["VVV"]=[x[0] for x in rrr[rr]["VVV"]]

with open(f'data/all_A_V_D7.txt', 'w', encoding='utf-8', newline='\r\n') as f:
