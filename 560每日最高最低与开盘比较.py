...
for dd in ddd:
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1":  # 除指数外，全部统计
        ...
        if ooo!=[] and (lenn:=len(ooo))>=6:
            for oo in ooo[5:]:
                EA=int((float_000(oo[3])-float_000(oo[2]))*1000/float_000(oo[2]))
                EV=int((float_000(oo[4])-float_000(oo[2]))*1000/float_000(oo[2]))
                if EA not in rrr.keys():
                    rrr[EA]=0
                rrr[EA]+=1
                if EV not in rrr.keys():
                    rrr[EV]=0
                rrr[EV]+=1
                n+=1
                
rrr=sorted(rrr.items(), key=lambda x: x[0])
for rr in rrr:
    print(rr)
