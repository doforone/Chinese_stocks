...
# 统计第一次连续两日上涨后，第三日上涨与下跌的数量，不考虑交易成本。
...
for dd in ddd:
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1":  # 除指数外，全部统计
        ...
        if ooo!=[] and (lenn:=len(ooo))>=4:
            i=2
            while i<lenn-1:
                if float_000(ooo[i-2][12])<0 and float_000(ooo[i-1][12])>0 and float_000(ooo[i][12])>0:  # 第一次连续两日上涨
                    if float_000(ooo[i+1][12])>0:  # 第三日上涨
                        rrr["A"]+=1
                        i+=3
                    elif float_000(ooo[i+1][12])<0:  # 第三日下跌
                        rrr["V"]+=1
                        i+=2
                i+=1


print(f"第三日上涨：{rrr['A']}，下跌：{rrr['V']}，上涨与下跌之比为：{round(rrr['A']/rrr['V'],2)}")
print("--end--")
