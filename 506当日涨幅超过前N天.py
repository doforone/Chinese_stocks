...
print("说明：1、数据使用为不复权数据；2、当日涨跌幅度=(当日收盘-前日收盘)-1；3、每年分红股票（●）为至少上市4年，最多1年没有分红（除权）的股票；4、新上市股票不定期更新。\r\n")
...
##for rr in rrr[:50]:
##    if rr[0][:6]=="sh.688":
##        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨次数：{round(rr[3]*100,2)}%，科创板")
##    elif rr[0][:4]=="sz.3":
##        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨次数：{round(rr[3]*100,2)}%，创业板")
##    else:
##        print(f"{rr[0]} {rr[1]}（{rr[2]}） 连续上涨次数：{round(rr[3]*100,2)}%")

print("========当日涨幅超过前N天排名（前12）========\r\n")
for rr in rrr[::-1][:12]:
    if rr[0][:6]=="sh.688":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 当日涨幅超过前：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  科创板")
    elif rr[0][:4]=="sz.3":
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 当日涨幅超过前：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}  创业板")
    else:
        print(f"{rr[0]} {rr[1]}（{rr[2]}） 当日涨幅超过前：{rr[3]}天，数据截止：{rr[4]}，收盘价：{rr[5]}元  {'●' if rr[6]==1 else ''}")
print("......")
