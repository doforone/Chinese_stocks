    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1":
        ...
        if ooo!=[] and len(ooo)>5:
            if dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
                AA=sum(float_000(oo[12])>=9.8 for oo in ooo)  # 普通版
            else:
                AA=sum(float_000(oo[12])>=19.8 for oo in ooo)  # 科创板
            rrr1.append([dd, ooo[-1][5], AA, len(ooo)])

            if dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":
                VV=sum(float_000(oo[12])<=-9.8 for oo in ooo)  # 普通版
            else:
                VV=sum(float_000(oo[12])<=-19.8 for oo in ooo)  # 科创板
            rrr2.append([dd, ooo[-1][5], VV, len(ooo)])


rrr1=sorted(rrr1, key=lambda x: x[2]/x[3], reverse=True)
print(f"\n涨停比例最高排名（前20）")
for rr in rrr1[:20]:
    print(f"{rr[0][0]} {rr[0][1]}  {rr[3]}天  {rr[1]}元  {round(rr[2]*100/rr[3], 2)}%")

##print(f"\n涨停比例最小排名（前20）")
##for rr in rrr1[-20:]:
##    print(f"{rr[0][0]} {rr[0][1]}  {rr[3]}天  {rr[1]}元  {round(rr[2]*100/rr[3], 2)}%")

rrr2=sorted(rrr2, key=lambda x: x[2]/x[3], reverse=True)
print(f"\n跌停比例最高排名（前20）")
for rr in rrr2[:20]:
    print(f"{rr[0][0]} {rr[0][1]}  {rr[3]}天  {rr[1]}元  {round(rr[2]*100/rr[3], 2)}%")
...
print("备注：1、数据均使用不复权数据；2、主板按9.8%涨跌比例计算，科创板按19.8%涨跌比例计算。")
print("--end--")
