...
for dd in ddd:
    ...
    if dd[0][:3]!="of." and dd[4]=="1" and dd[5]=="1" and dd[0][:6]!="sh.688" and dd[0][:4]!="sz.3":  # 不统计科创板
        ...
        if ooo!=[]:
            s0=0
            for oo in reversed(ooo):
                if oo[0]=="2023-04-03":
                    s0=f0(oo[5])
                    break

            if s0>0:
                rrr.append([dd[0], dd[1], dd[2], ooo[-1][0], f0(ooo[-1][5]), s0])  # 股票代码 名称 上市日期，收盘价


rrr.sort(key=lambda x: x[4]/x[5], reverse=True)  # 按股价排序

rrr2={}
for rr in rrr:
    #print(f"{rr[0]}  {rr[1]}  上市：{rr[2]}  收盘：{rr[3]}  开始价：{rr[5]}元  收盘价：{rr[4]}元")
    key=rr[5]//5  # 股价按每10元汇总
    rrr2.setdefault(key, 0)
    if rr[4]>rr[5]:
        #rrr2[key]+=1
        rrr2[key]+=(rr[4]/rr[5]-1)
    elif rr[4]<rr[5]:
        #rrr2[key]-=1
        rrr2[key]-=(1-rr[4]/rr[5])
    

rrr2=sorted(rrr2.items(), key=lambda x: x[0])
print(f"股票统计总数：{len(rrr)}")
for rr in rrr2:
    print(f"{int(rr[0]):>3}——{int((rr[0]+1)*5):>4}元：{rr[1]}支")
